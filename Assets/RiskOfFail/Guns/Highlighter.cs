...
public class Highlighter : Weapon
{
    ...
    private IEnumerator ShootSustain(Transform player, PlayerShooting shooting, float multiplier = 1)
    {
        while (KInputManager.GetKey("Shoot").Pressed() && shooting.GetHeldWeapon().clip > 0)
        {
            // raycast
            var mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
            var hit = Physics2D.Raycast(player.position, (mousePosition - player.position).normalized,
