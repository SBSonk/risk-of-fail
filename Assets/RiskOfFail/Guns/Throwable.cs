...
public abstract class Throwable : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        var normalizedProgress = duration / distance * speed;

        // Apply a curve to get the arc-like motion
        var curveValue = Mathf.Sin(normalizedProgress * Mathf.PI);

        // Calculate the vertical offset using the curve and height
        var yOffset = curveValue * height;

        // Calculate the new position using lerp
        var newPosition = Vector3.Lerp(startPosition, target, normalizedProgress);
