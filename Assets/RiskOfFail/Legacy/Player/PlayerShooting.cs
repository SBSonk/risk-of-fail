...
public class PlayerShooting : MonoBehaviour
{
    ...
    [Header("Shoving")] [SerializeField] private float shoveStrength;

    [SerializeField] private float shoveStunTime = 1f;
    [SerializeField] private float shoveCooldown = 1f, shoveHitDelay = 0.25f;
    ...
    private Vector2 playerOffset = new(0, 0.5f);
    private float shootEnableTime;
    private InventoryWeapon[] weaponPool;
    ...
    private void Update()
    {
        ...
        // Reloading
        if (GetHeldWeapon().clip < GetHeldWeapon().weapon.clipSize && KInputManager.GetKey("Reload").PressedDown() &&
    ...
    private void WeaponPickups()
    {
        if (weaponsInArea.Count == 0) return;

        var nearestIndex = 0;
        float nearestDistance = 0;
        for (var i = 0; i < weaponsInArea.Count; i++)
        {
            if (i == 0)
            {
            ...
            var dist = Vector2.Distance(transform.position, weaponsInArea[i].transform.position);
    ...
    {
        return Time.time >= shootEnableTime && canShoot;
    }

    private bool CheckIfPlayerShooting()
    {
        if (GetHeldWeapon().weapon.auto) return KInputManager.GetKey("Shoot").Pressed();
        return KInputManager.GetKey("Shoot").PressedDown();
    }

    private void WeaponSwitching()
    {
        var startIndex = currentWeaponIndex;
        var swapDir = 0;
    ...
    private void Shoving()
    {
        ...
        var shoveDir = ((Vector3)mousePosition - transform.position).normalized;
        var hits = Physics2D.OverlapCircleAll(transform.position + shoveDir * radius, radius);
        ...
        foreach (var h in hits)
            ...
            else if (h.gameObject.layer == LayerMask.NameToLayer("EnemyProjectile"))
            {
                foreach (var c in hits)
                    ...
                    {
                        // Spawn Bullet
                        var bullet = Instantiate(friendlyQuizBullet, gunBarrel.position, gunBarrel.rotation);
        ...
        var cooldownTime = shoveCooldown + GetHeldWeapon().weapon.reloadLength / 4;
    ...
    private void GunBehavior(Gun g)
    {
        ...
        if (g.clipSize > 0)
        {
            ...
            if (weapon.clip == 0 && !reloading) StartCoroutine(Reload());
        }
        else
        {
    ...
    private void HighlighterBehavior(Highlighter g)
    {
        // Shooting and magazines
        var weapon = GetHeldWeapon();
        if (g.clipSize > 0)
    ...
    private void MeleeBehavior(MeleeWeapon m)
    {
        // Shooting and magazines
        var weapon = GetHeldWeapon();
        if (m.clipSize > 0)
    ...
    private IEnumerator Reload()
    {
        ...
        var reloadTime = heldWep.weapon.reloadLength / reloadMultiplier;
        ...
        else
        {
            ...
            if (reloading)
            {
                var amountToReload = heldWep.weapon.clipSize;
    ...
    public bool InventoryFull()
    {
        ...
        for (var i = 0; i < weaponPool.Length; i++)
            if (weaponPool[i].weapon != null)
                count++;
    ...
    {
        var weaponInSlot = weaponPool[weaponIndex];
