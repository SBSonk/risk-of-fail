...
public class Stapler : MeleeWeapon
{
    ...
    protected virtual GameObject SpawnBullet(Transform player, float multiplier = 1)
    {
        ...
        Vector3 mousePosition = Camera.main.ScreenToWorldPoint(Input.mousePosition);
        Vector3 hitVector = (mousePosition - player.position).normalized;
        var projectile = Instantiate(bullet, position: player.position + hitVector + bulletOffset, rotation: player.rotation);
        
        float x = mousePosition.x - projectile.transform.position.x;
        float y = mousePosition.y - projectile.transform.position.y;
