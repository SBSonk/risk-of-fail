...
public class CameraFollow : MonoBehaviour
{
    ...
    [SerializeField] private float lerpVal = 0.15f, roomViewLerp = 0.05f, maxCameraPredict = .5f;

    [Header("RoomTransfer")] [SerializeField]
    private CameraBounds worldBoundsX, worldBoundsY;
    ...
    private Vector3 desiredPos, lastPlayerVel;
    private Rigidbody2D player;
    private float startCameraSize;
    private Vector3 startOffset;
    private CameraBounds startX, startY;
    ...
    private void FixedUpdate()
    {
        ...
        var lValue = lockToCenter ? roomViewLerp : lerpVal;
        ...
        var xPlayerVel = KeyBind.GetAxis(KInputManager.GetKey("Right"), KInputManager.GetKey("Left")) != 0
            ? player.linearVelocity.x
            : lastPlayerVel.x;
        var yPlayerVel = KeyBind.GetAxis(KInputManager.GetKey("Up"), KInputManager.GetKey("Down")) != 0
    ...
    public IEnumerator CenterCameraOnPosition(CenterCameraOnObject c, Vector3 position, float time, float holdTime,
        Vector3 offset, bool freezeGame)
    {
    ...
    public IEnumerator CenterCameraOnPosition(CenterCameraOnObject c, Vector3[] positions, float time, float holdTime,
        Vector3 offset, bool freezeGame = false)
    {
        c.OnCenterStart?.Invoke();
        if (freezeGame) Time.timeScale = 0;
        ...
        int i;
        for (i = 0; i < positions.Length; i++) position += positions[i];
        position /= i;
