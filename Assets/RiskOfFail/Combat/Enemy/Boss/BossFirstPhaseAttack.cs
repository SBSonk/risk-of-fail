...
public class BossFirstPhaseAttack : MonoBehaviour
{
    ...
    [Header("Idle")] [SerializeField] private float idleBufferTime = 3;

    [Header("ChoosingAttack")] [SerializeField]
    private float chooseBufferTime = 1;
    ...
    private void SwitchState(BossState state)
    {
        ...
        if (currentState == BossState.ChoosingAttack)
        {
            StartCoroutine(ChooseAttackPhase());
        }
    ...
    private void EnterPreDamagePhase()
    {
        attacksDone = 0;
    ...
    private void CancelPreDamagePhase()
    {
    ...
    private void EndDamagePhase()
    {
        ...
        for (var i = 0; i < itemSpawners.Length; i++) itemSpawners[i].SpawnItems();
    }

    private BossAttack ChooseAttack()
    {
        var colliders =
            Physics2D.OverlapBoxAll(col.bounds.center, col.bounds.size * 1.5f, 0, LayerMask.GetMask("Player"));

        if (colliders.Length > 0) return closeRangeAttack;

        if (attacksBeforeSpecial <= 0)
        {
            var randomIndex = Random.Range(0, specialAttacks.Length);
            attacksBeforeSpecial = Random.Range(minAmountForSpecial, maxAmountForSpecial);
            return specialAttacks[randomIndex];
        }
        else
        {
            var randomIndex = Random.Range(0, randomAttacks.Length);
            attacksBeforeSpecial--;
            return randomAttacks[randomIndex];
        }
    }

    private IEnumerator ChooseAttackPhase()
    {
        spawner.UseAttack();
